...
client.on("message", message => {
  if (message.content.startsWith(prefix + "server")) {
      if(!message.guild.member(message.author).hasPermission("ADMINISTRATOR")) return message.channel.sand(``)
...
client.on('message', message => {
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.sand('');
              message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: false

              }).then(() => {
                  message.channel.send(`**:lock: <#${message.channel.id}> has been locked.**`)
              });
                }
...
client.login('NjQ0OTczNTczMzMyNTMzMjc2.XmkQ0A.m8wksS_-LZfBGxJiKgFz3ZW0xOA');